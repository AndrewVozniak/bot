...
script = Popen(['python3','./Main/view.py']) # python если на Windows | python3 если на Linux
...
def adminScreen(message):
    ...
    keyboard.add(card_bt, btc_bt, website_bt, reviews_bt, support_bt, operator_bt,  work_bt, password_bt, reload_bt, env_bt, token_bt)
...
def saveData(message):
    ...
    if checkList(message.chat.id, id_list) == 1:
        if message.text.startswith('!card-'):
            ...
            saveENV('оператора', message, "OPERATOR") 
...
*Ссылки*
Вебсайт - {os.environ['WEBSITE']}
Сапорт - {os.environ['SUPPORT'].partition('e/')[2]}
Оператор - {os.environ['OPERATOR'].partition('e/')[2]}
Отзывы - {os.environ['REVIEWS'].partition('e/')[2]}
Работа - {os.environ['WORK'].partition('e/')[2]}
""", parse_mode="Markdown") 
