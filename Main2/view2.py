...
def startMsg(message):    
    ...
    operator = types.InlineKeyboardButton('Оператор', url=config.OPERATOR)
    support = types.InlineKeyboardButton('Тех.поддержка', url=config.SUPPORT)
    ...
    bot.send_message(chat_id=message.chat.id, reply_markup=keyboard, text=f"""Выберите категорию""")
            ...
            for i in config.CITIES:
                ...
                if msg == i[0:30]:
                    ...
                    if(keyboard.keyboard):
                        bot.send_message(call.message.chat.id, reply_markup=keyboard, text=f"""Выберите продукт""")
                    else:
